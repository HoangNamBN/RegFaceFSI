...
def train(path_train,model_save_path=None,verbose=False):
    ...
    if model_save_path is not None:
        with open(model_save_path, 'wb') as f:
            pickle.dump([clf,encodings,names], f)
    return clf, encodings, names
CountDetect = 0
timer = 0
def show_prediction_labels_on_image(X_img_path,path_out,encodings,names,model_save_path=None,clf=None):
    global CountDetect
    global timer
    ...
    for (top, right, bottom, left), face_encoding, pred in zip(X_face_locations, faces_encodings, pred):
...
if __name__ == "__main__":
    ...
    if os.path.isfile(model_save_path):
        with open(model_save_path, 'rb') as f:
            clf, encodings, names = pickle.load(f)
    else:
        print( "Training SVM classifier...")
        start = time.time()
        clf, encodings, names = train(path_train,model_save_path)
        end = time.time()
    ...
    for image_file in os.listdir(path_test):
        ...
        name = show_prediction_labels_on_image(full_file_path,path_out, encodings, names,model_save_path)
